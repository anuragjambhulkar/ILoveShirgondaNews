...
async function handleClearNotifications() {
  try {
    ...
    return handleCORS(NextResponse.json({ message: 'Notifications cleared' }));
  } catch (error) {
...
async function handleGetCategories() {
  try {
    ...
    let categories = await categoriesCollection.find({}).sort({ order: 1 }).toArray();
    ...
    return handleCORS(NextResponse.json({ categories: fallbackCategories }));
  }
}
...
async function handleRoute(request, { params }) {
  ...
  try {
    ...
    if (route === '/notifications') {
      if (method === 'GET') return handleGetNotifications();
      if (method === 'DELETE') return handleClearNotifications();
    }
